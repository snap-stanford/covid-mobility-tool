...
class Model:
    ...
    def init_exogenous_variables(self,
                                 ...
                                 latency_period=LATENCY_PERIOD_IN_DAYS*24,
                                 infectious_period=INFECTIOUS_PERIOD_IN_DAYS*24, 
                                 confirmation_rate=DETECTION_RATE,
                                 confirmation_lag=DETECTION_LAG*24,
        ...
        if self.POI_CBG_VISITS_LIST is not None:
            ...
            assert self.POI_TIME_COUNTS is not None
        ...
        if self.cbg_day_prop_out is not None:
            ...
            assert(num_days == self.cbg_day_prop_out.shape[1])
            print('Received daily proportions out per CBG')
        self.HOME_BETA = home_beta
        if type(self.HOME_BETA) in {np.ndarray, list}:  # received time-varying home beta, should be daily
            assert len(self.HOME_BETA) == num_days
            print('Received daily home beta from %.3f to %.3f' % (self.HOME_BETA[0], self.HOME_BETA[-1]))
        
        self.inter_cbg_travel = inter_cbg_travel
        self.GAMMA = inter_cbg_gamma
        if self.inter_cbg_travel is not None:
            assert(self.N == self.inter_cbg_travel.shape[0])
            assert(num_days == self.inter_cbg_travel.shape[1])
            print('Received daily inter-CBG travel per CBG')
        ...
        self.daily_mask_use = daily_mask_use
        self.mask_efficacy = mask_efficacy
        if self.daily_mask_use is not None:
            assert len(self.daily_mask_use) == int(self.T / 24)  # num days
            print('Received daily mask use data, will use it to scale transmission rates (along with mask efficacy = %s)' % self.mask_efficacy) 

    def _build_proportions_matrix(self, poi_cbg_proportions):
        '''
        ...
        if type(poi_cbg_proportions) == np.ndarray:
            assert poi_cbg_proportions.shape == (self.M, self.N)
            self.POI_CBG_PROPORTIONS = poi_cbg_proportions
        else:  # should be a list of dictionaries
            assert type(poi_cbg_proportions) == list
            assert len(poi_cbg_proportions) == self.M
            assert type(poi_cbg_proportions[0]) == dict
            self.POI_CBG_PROPORTIONS = np.zeros((self.M, self.N))
            for poi, cbg_dict in enumerate(poi_cbg_proportions):
    ...
    def init_endogenous_variables(self):
        if self.initial_conditions is None:
            ...
            assert self.initial_conditions.shape == (self.N, 3)
            print('Received initial conditions per CBG')
            ones = np.ones((self.num_seeds, self.N))
            self.cbg_latent = np.random.poisson(self.initial_conditions[:, 0] * ones)  # repeat rows
            self.cbg_infected = np.random.poisson(self.initial_conditions[:, 1] * ones)
            self.cbg_removed = np.random.poisson(self.initial_conditions[:, 2] * ones)
            self.initial_conditions = None  # too large to save with model; already have saved version
    ...
    def simulate_disease_spread(self, verbosity=24,
        ...
        self.use_inter_cbg_leak_factor = use_inter_cbg_leak_factor
        if self.use_inter_cbg_leak_factor:
            print('Using inter-CBG travel patterns as additional leak factor')
            assert self.inter_cbg_travel is not None
        ...
        while t < self.T:
            ...
            if allow_early_stopping and np.max(self.cbg_latent + self.cbg_infected) < 1:
    ...
    def get_new_cases(self, t):
        ...
        day = int(t / 24)
        if self.daily_mask_use is not None:
            curr_mask_use = self.daily_mask_use[day]
            scaling_factor = (1 - (curr_mask_use * self.mask_efficacy)) ** 2
        else:
            scaling_factor = 1
        if type(self.HOME_BETA) in {np.ndarray, list}:
            beta = self.HOME_BETA[day]
        else:
            beta = self.HOME_BETA
        ...
        if self.PSI > 0:  # Our model: can only be infected by people in your home CBG.
            if self.use_home_proportion_beta:
                ...
                cbg_base_infection_rates = beta * cbg_densities  # S x N
        else:
        ...
        if self.use_aggregate_mobility:  # don't use network data, just use total number of POI visits in this hour
            ...
            cbg_agg_poi_infection_rates = cbg_agg_poi_infection_rates * scaling_factor
        ...
        if self.use_inter_cbg_leak_factor:
            ...
            cbg_leak_infection_rates = cbg_leak_infection_rates * self.inter_cbg_travel[:, day]  # broadcast over seeds
            num_to_clip = np.sum(cbg_leak_infection_rates > 1)
            if (num_to_clip / self.N) > 0.05:
                print('Warning: need to clip more than 5% of CBGs for inter-CBG leak infection rate')
            cbg_leak_infection_rates = np.clip(cbg_leak_infection_rates, None, 1)
            num_sus_remaining = num_sus - self.cbg_new_cases 
            cases_from_cbg_leak = np.random.binomial(num_sus_remaining.astype(int), cbg_leak_infection_rates)
            self.cbg_new_cases = self.cbg_new_cases + cases_from_cbg_leak
